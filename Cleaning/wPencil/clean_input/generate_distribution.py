...
# LHC 2017 starting at IP3:dist_generator(10000, 6.5e12, 'HL_coll', 'False', 1, 1, 3.5e-6, 3.5e-6, 1.724355, -2.059930, 131.520626, 144.694088, 0, 0, 0, 0, 1.876058, -0.024731, 0.062690, 0.002870, 42, 0, 1)
...
def get_rel_params(refMom, mass=0.938272046e9):
    """
...
def get_bucket(machine, plot=True, z=0, DELTA=0):
    ...
    if machine == 'LHC_coll' or machine == 'HL_coll_tcp':
...
def dist_generator(particles, refMom, energy, machine, fort13, jobs, factor, emittance_x, emittance_y, alpha_x, alpha_y, beta_x, beta_y, offset_x, offset_xp, offset_y, offset_yp, dispersion_x, dispersion_xp, dispersion_y, dispersion_yp, bunch, spread, seed):

    job_str = '%s'%jobs
    # Getting the Transverse sigmas (amplitudes of phase space ellipse)
    # --------------------------------------------------------------------------------------------------------------
    gamma_rel, beta_rel, p0, mass = get_rel_params(refMom)
    ...
    # Generat ing the Transverse Distribution
    # --------------------------------------------------------------------------------------------------------------
	...
	#print len(z)
        ...
        dPP     = (trial_p - p0) / p0
	dEE	= (trial_e - energy) / energy
#        h       = get_bucket(machine, plot=False, z=trial_z, DELTA=dPP)  # Longitudinal contour
        h       = get_bucket(machine, plot=False, z=trial_z, DELTA=dEE)  # Longitudinal contour, for displaced bucket due to RF trim
	...
	if abs(x[i]*1e3) > 3.8:
		...
		y_cut.append(y[i])
		yp_cut.append(yp[i])
		E_cut.append(EE[i]*1e-6)
		z_cut.append(z[i]*1e3)
		ddp_cut.append(ddp[i])

    count = 0
    for i in x:
	if abs(i*1e3) > 3.8:
		count += 1    
    print "Particles beyond cut =", count
    ...
    #plot(x,xp,'.')
    #show()
    return (x,xp,y,yp,zz,ddp,EE,x_cut,xp_cut,y_cut,yp_cut,E_cut,z_cut,ddp_cut)
if __name__=="__main__":
    # Call the function
    # --------------------------------------------------------------------------------------------------------------
